...
def test_set_and_get_secret_fallback():
    ...
    prefs = {}
    cs.set_secret("google_client_secret", "secret-abc", prefs)
    assert prefs["google_client_secret"] == "secret-abc"
    assert cs.get_secret("google_client_secret", prefs) == "secret-abc"
...
def test_missing_secret_returns_empty():
    cs = _store_with_fallback()
    assert cs.get_secret("google_client_secret", {}) == ""
    assert cs.get_secret("onedrive_token", {}) == {}


def test_with_secrets_overlays_resolved_values():
    cs = _store_with_fallback()
    prefs = {"google_client_secret": "", "onedrive_client_id": "cid"}
    cs.set_secret("google_client_secret", "tok", prefs)
    merged = cs.with_secrets(prefs)
    assert merged["google_client_secret"] == "tok"
...
def test_delete_secret_fallback():
    cs = _store_with_fallback()
    prefs = {}
    cs.set_secret("google_client_secret", "x", prefs)
    cs.delete_secret("google_client_secret", prefs)
    assert "google_client_secret" not in prefs
